...
def train_model(data_yaml_path, model_version):
    ...
    model = YOLO("src/yolo/weights/best.pt")
...
def main():
    ...
    with mlflow.start_run():
        ...
        print("\n[2/5] Creating train/val split (90/10)...")
        if DATA_DIR.exists():
        ...
        print("\n[3/5] Creating data.yaml...")
        data_yaml_path = create_data_yaml()
        print(f"  Saved to: {data_yaml_path}")

        print("\n[4/5] Training model (300 epochs with augmentations)...")
        ...
        train_model(data_yaml_path, model_version)
